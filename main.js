    ...
    $('#addPersonBtn').click(function () {
        ...
        $('#cvv').val('');            
            ...
            for (var i = 0; i < people.length; i++) {
                var person = people[i];
                $('tbody').append('<tr><td>' + person.Name + '</td><td>' + person.Email + '</td><td>' + person.Tel + '</td><td>' + person.Address + '</td><td>' + person.Zip + '</td><td>' + person.City + '</td><td>' + person.State + '</td><td>' + person.Country + '</td><td>' + person.Type + '</td><td>' + person.Num + '</td><td>' + person.Exp + '</td><td>' + person.Cvv + '</td></tr>');
            }
        }
        else {
            if (!$('#nameContainer').find('#nameError').length) {
                $("input[id='name']").after('<br /><p id="nameError">Please provide all info.</p>');
            }
        }
    });
...
let countDown = new Date('Oct 19, 2017 10:00:00').getTime(),
